...
Animations normally run when toggling via keyboard, but you can opt into animating pointer-triggered collapse/expand (dragging past the collapse buffer) by adding the `collapseAnimationOnPointer` prop (default `false`).
Try focus one of the resizer handles and pressing `Enter`, or drag to collapse/expand, to see the animation in action.
